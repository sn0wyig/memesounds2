 ...
 * @authorLink https://github.com/sn0wyig/
 * @source https://github.com/sn0wyig/BetterDiscordPlugins/blob/main/MemeSounds/MemeSounds.plugin.js
 * @updateUrl https://raw.githubusercontent.com/sn0wyig/BetterDiscordPlugins/main/MemeSounds/MemeSounds.plugin.js
 */
...
module.exports = (() => {
	...
	return !global.ZeresPluginLibrary ? class {
		...
		const plugin = (Plugin, Api) => { try {
			...
			const sounds = [
				{re: /so cool/gmi, file: "socool.mp3", duration: 600},
				...
				messageEvent = async ({ channelId, message, optimistic }) => {
					...
					if (!optimistic && lastMessageID != message.id) {
						...
						for (let sound of [...queue.entries()].sort((a, b) => a[0] - b[0])) {
							let audio = new Audio("https://github.com/sn0wyig/BetterDiscordPlugins/blob/main/MemeSounds/Sounds"+sound[1].file);
